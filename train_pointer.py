...
"""
使用示例:
    python train_pointer_fixed.py --use_direction --epochs 60
"""
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'train':
            ...
            if S > 1:
                ...
                page['sentence_labels'] = page['sentence_labels'][sent_perm]  
            ...
            if N > 1:
                ...
                page['char_labels'] = page['char_labels'][char_perm]  
...
class PointerNetwork(nn.Module):
    ...
    def attention(self, query, keys, mask=None):
        """
        计算attention权重
        query: [hidden_dim]
        keys: [N, hidden_dim]
        ...
        scores = self.v(torch.tanh(
            self.W1(keys) + self.W2(query_expanded)
        )).squeeze(-1)
    ...
    def forward(self, batch: Dict) -> Dict:
        ...
        # 句子级：编码 + 贪婪解码
        sent_enc, _ = self.sent_encoder(s_feat.unsqueeze(0))
        sent_enc = sent_enc.squeeze(0)  # [S, hidden_dim]
        
        mask = torch.ones(S, device=device)
        query = torch.zeros(self.hidden_dim, device=device)
        sent_scores = torch.zeros(S, device=device)
        
        for step in range(S):
            probs = self.attention(query, sent_enc, mask)
            selected = torch.argmax(probs)
            sent_scores[selected] = float(step)  # ✅ 分数递增（越小越靠前）
            mask[selected] = 0
            query = sent_enc[selected]
        
        # 字符级：编码 + 贪婪解码
        char_enc, _ = self.char_encoder(c_feat.unsqueeze(0))
        char_enc = char_enc.squeeze(0)  # [N, hidden_dim]
        ...
        for step in range(N):
            probs = self.attention(query, char_enc, mask)
            selected = torch.argmax(probs)
            char_scores[selected] = float(step)  # ✅ 分数递增（越小越靠前）
            mask[selected] = 0
            query = char_enc[selected]
    ...
    def training_forward(self, batch: Dict) -> Dict:
        sb = batch['sentence_bboxes']
        sd = batch['sentence_dir_feat']
        cb = batch['char_bboxes']
        cd = batch['char_dir_feat']
        ...
        if self.use_direction:
            s_feat = torch.cat([s_geom, sd], dim=1)
            c_feat = torch.cat([c_geom, cd], dim=1)
        else:
            s_feat = s_geom
            c_feat = c_geom
        ...
        # 句子级：编码
        sent_enc, _ = self.sent_encoder(s_feat.unsqueeze(0))
        sent_enc = sent_enc.squeeze(0)
        
        # Teacher forcing：按真实顺序选择
        sent_order = torch.argsort(sl)
        sent_log_probs = []
        mask = torch.ones(sb.shape[0], device=device)
        query = torch.zeros(self.hidden_dim, device=device)
        
        for idx in sent_order:
            probs = self.attention(query, sent_enc, mask)
            sent_log_probs.append(torch.log(probs[idx] + 1e-10))
            mask[idx] = 0
            query = sent_enc[idx]
        
        # 字符级：编码
        char_enc, _ = self.char_encoder(c_feat.unsqueeze(0))
        char_enc = char_enc.squeeze(0)
        
        # Teacher forcing
        char_order = torch.argsort(cl)
        char_log_probs = []
        mask = torch.ones(cb.shape[0], device=device)
        query = torch.zeros(self.hidden_dim, device=device)
        
        for idx in char_order:
            probs = self.attention(query, char_enc, mask)
            char_log_probs.append(torch.log(probs[idx] + 1e-10))
            mask[idx] = 0
            query = char_enc[idx]
        
        return {
            'sent_log_probs': torch.stack(sent_log_probs),
            'char_log_probs': torch.stack(char_log_probs)
        }


# ============================== Loss ==============================
def pointer_loss(log_probs: torch.Tensor) -> torch.Tensor:
...
def evaluate(model: nn.Module, loader: DataLoader, device: torch.device) -> Dict[str, float]:
    ...
    for batch in tqdm(loader, desc='Evaluating', leave=False):
        ...
        # Sentence-level
        s_idx = torch.argsort(sent_scores, dim=0)  # ✅ 升序（与JointModelV4一致）
        ...
        for sid in torch.unique(cs).tolist():
            ...
            c_sorted = torch.argsort(char_scores[idx], dim=0)  # ✅ 升序
...
def train(cfg: TrainConfig):
    ...
    save_dir = Path(cfg.save_dir) / f"pointer_fixed_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch}/{cfg.epochs}"):
            ...
            loss_s = pointer_loss(out['sent_log_probs'])
            loss_c = pointer_loss(out['char_log_probs'])
            loss = loss_s + loss_c
